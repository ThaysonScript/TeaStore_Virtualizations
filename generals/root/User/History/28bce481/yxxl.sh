...
for index in ${TOMCATS_ARRAY[@]}; do
    ...
    export CATALINA_HOME="$CATALINA_HOME/$index"; java -jar $CATALINA_HOME/bin/dockermemoryconfigurator.jar ${TOMCAT_HEAP_MEM_PERCENTAGE};
    export CATALINA_HOME="$CATALINA_HOME/$index"; bash $CATALINA_HOME/bin/start.sh $CATALINA_HOME/$index
    export CATALINA_HOME="$CATALINA_HOME/$index"; bash $CATALINA_HOME/bin/catalina.sh run
